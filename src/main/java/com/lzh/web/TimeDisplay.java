...
public class TimeDisplay extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
	    ...
	    int end = Integer.parseInt(request.getParameter("end").toString());
        ...
        String sqlQuery = "select t_long,t_lat from "+layer+" where t_long>="+xmin+" and t_long<="+xmax+" and t_lat>="+ymin+" and t_lat<="+ymax+" and t_time>"+start+" and t_time<"+end;
        if(len == 1){
        	if(state_array[0].equals("0")){
        		sqlQuery = "select t_long,t_lat from "+layer+" where t_long>="+xmin+" and t_long<="+xmax+" and t_lat>="+ymin+" and t_lat<="+ymax+" and t_time>"+start+" and t_time<"+end+" and t_state=1";
        	}
        	else if(state_array[0].equals("1")){
        		sqlQuery = "select t_long,t_lat from "+layer+" where t_long>="+xmin+" and t_long<="+xmax+" and t_lat>="+ymin+" and t_lat<="+ymax+" and t_time>"+start+" and t_time<"+end+" and t_state=17";
        	}
        }
